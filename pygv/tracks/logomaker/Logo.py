...
from pygv.tracks.logomaker.Glyph import Glyph
from pygv.tracks.logomaker.validate import validate_matrix
from pygv.tracks.logomaker.error_handling import check, handle_errors
from pygv.tracks.logomaker.colors import get_color_dict, get_rgb
from pygv.tracks.logomaker.matrix import transform_matrix
...
class Logo:
    ...
    **kwargs:
    ...
    def draw(self, clear=False):
        ...
        xmin = min([g.p for g in self.glyph_list])
        xmax = max([g.p for g in self.glyph_list])
        self.ax.set_xlim([xmin, xmax])
        # draw baseline
        # self.draw_baseline(linewidth=self.baseline_width)
        self.ax.plot([xmin, xmax], [0, 0], lw=self.baseline_width)
